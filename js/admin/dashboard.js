      ...
      if (admins.length == 0) {
        ...
        admins.forEach((admin) => {
          html +=
            `<li class="item">
                  <a href="` +
            url("admin/users/" + admin.id + "/edit") +
            `" class="text-white">
                    <i class="fas fa-check-circle text-success"></i> <p class="d-inline">` +
            admin.name +
            `</p>
                  </a>
                  </li>`;
...
function income_chart_statistics() {
  ...
  $.ajax({
    url:
      "get_income_chart?month=" +
      month +
      "&year=" +
      year +
      "&branch_id=" +
      $("#filter_income_branch").val(),
    beforeSend: function () {
      $(".preloader").show();
      $(".loader").show();
    },
    success: function (response) {
      ...
      $("#income_chart").append(
        `
          <canvas id="income_chart_statistics"  height="` +
          height +
          `" width="80vw"></canvas>
        `
      );
...
function filter_statistics() {
  ...
  $.ajax({
    url: "/ajax/get_statistics?date=" + encodeURIComponent(date),
    beforeSend: function () {
      $(".preloader").show();
      $(".loader").show();
    },
