    ...
    var antibiotic_count_mc=$('#antibiotic_count_mc').val();
    ...
    (function($){
        ...
        table=$('#medical_reports_table').DataTable( {
            ...
            "ajax": {
                ...
                data:function(data)
                {
                    ...
                    data.filter_result=$('#filter_result').val();
                    data.filter_uploaded_report=$('#filter_uploaded_report').val();
                    data.filter_sent_status=$('#filter_sent_status').val();
                    data.filter_barcode=$('#filter_barcode').val();
                    data.filter_date=$('#filter_date').val();
                    data.filter_date2=$('#filter_date2').val();
                    data.filter_created_by=$('#filter_created_by').val();
                    data.filter_signed_by=$('#filter_signed_by').val();
                    data.filter_signed_by2=$('#filter_signed_by2').val();
                    data.filter_contract=$('#filter_contract').val();
                    data.filter_called=$('#filter_called').val();
                }
            },
            fixedHeader: true,
            "timeout": 15000, //bp
            "columns": [
                ...
                {data:"updated_at",searchable:false,orderable:true,sortable:true},
               
                {data:"done",searchable:false,sortable:false,orderable:false},
                {data:"sent",searchable:false,sortable:false,orderable:false},
               
                {data:"signed",searchable:false,sortable:false,orderable:false},
                {data:"signed2",searchable:false,sortable:false,orderable:false},
        ...
        $('#filter_sent_status').on('change',function(){
            table.draw();
        });
        ...
        $('#filter_called').on('input',function(){
            table.draw();
        });
        ...
        $('#filter_date').daterangepicker({
         ...
         $('#filter_date2').daterangepicker({
            ...
            $('#filter_date2').on( 'cancel.daterangepicker', function(){
            $(this).val('');
            });
        ...
        $('#filter_date2').on('apply.daterangepicker',function(){
            table.draw();
        });
        ...
        $('#filter_uploaded_report').on('change',function(){
            table.draw();
        });
        ...
        $('#filter_signed_by2').on('change',function(){
            table.draw();
        });
    ...
    $(document).on("change keyup input copy paste keydown",'.test_result',function(){
        var result=$(this).val();
        ...
        {
            $(this).closest('tr').find('.status').val('').trigger('change');
        }
...
//     $('.select_comment').on('select2:select', function(event) {
//         var comment = event.params.data;
//         var commentText = comment.text; // Check if the data structure is like this
//         $(this).closest('td').find('.comment').val(commentText);
//     });
// });
    ...
    //                         <option value="" disabled selected>`+trans("Select Sensitivity 1")+`</option>
    //                         <option value=".">`+trans(".")+`</option>
    //                         <option value="Sensitiv">`+trans("Sensitiv")+`</option>
    //                         <option value="Intermediar">`+trans("Intermediar")+`</option>
    //                         <option value="Rezistent">`+trans("Rezistent")+`</option>
    //                     </select>
    //                 </div>
    //             </td>
    //             <td>
    //                 <div class="form-group">
    //                     <select class="form-control sensitivity" name="antibiotic[`+antibiotic_count+`][sensitivity2]">
    //                         <option value="" disabled selected>`+trans("Select Sensitivity 2")+`</option>
    //                         <option value=".">`+trans(".")+`</option>
    //                         <option value="Sensitiv">`+trans("Sensitiv")+`</option>
    //                         <option value="Intermediar">`+trans("Intermediar")+`</option>
    //                         <option value="Rezistent">`+trans("Rezistent")+`</option>
    ...
    //     $('.sensitivity').select2({
    //         placeholder:trans('Select sensitivity 1'),
    //         width:'100%'
    //     });
        
    //      $('.sensitivity2').select2({
    //         placeholder:trans('Select sensitivity 2'),
    //         width:'100%'
    //     });
...
//         <option value="" disabled selected>`+trans("Select Group")+`</option>
//     `;
...
//     var row = $(el).closest('table').find('tbody').append(`
//         <tr>
//             <td>
//                 <div class="form-group">
//                     <select class="form-control categorySelect" name="antibiotic[`+antibiotic_count+`][category_id]" required>
//                         `+categories_options+`
//                     </select>
//                 </div>
//             </td>
...
//             <td>
//                 <div class="form-group">
//                     <select class="form-control sensitivity2" name="antibiotic[`+antibiotic_count+`][sensitivity2]">
//                         <option value="" disabled selected>`+trans("Select Sensitivity 2")+`</option>
//                         <option value=".">`+trans(".")+`</option>
//                         <option value="Sensitiv">`+trans("Sensitiv")+`</option>
//                         <option value="Intermediar">`+trans("Intermediar")+`</option>
...
//     var categorySelect = $(row).find('.categorySelect').last();
//     var antibioticSelect = $(row).find('.antibiotic').last();

//     $(categorySelect).change(function() {
//         var selectedCategory = $(this).val();

//         // Reset antibiotics dropdown
//         $(antibioticSelect).empty();

//         // Filter antibiotics based on selected category
//         var filteredAntibiotics = antibiotics.filter(function(antibiotic) {
//             return antibiotic.category_id == selectedCategory;
//         });

//         // Populate antibiotics dropdown with filtered antibiotics
//         filteredAntibiotics.forEach(function(antibiotic) {
//             $(antibioticSelect).append(`
//                 <option value="`+antibiotic.id+`">`+antibiotic.name+`</option>
//             `);
//         });
...
//         placeholder: trans('Select group'),
//         width: '100%'
//     });
...
function add_antibiotic(antibiotics, el) {
    ...
    var categorySelect = tbody.find('.categorySelect').last();
    $(categorySelect).change(function () {
        var selectedCategory = $(this).val();
        ...
        filteredAntibiotics.forEach(function (antibiotic) {
            var row = `
        <tr data-category="` + selectedCategory + `">
        <td class="col-3"></td>
            <td><span>` + antibiotic.name + `</span></td>
            <td>
                <div class="form-group">
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" id="sensitivityS` + antibiotic_count + `" name="antibiotic[` + antibiotic_count + `][sensitivity]" value="Sensitiv">
                        <label class="form-check-label sensitivityS" for="sensitivityS` + antibiotic_count + `">S</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" id="sensitivityI` + antibiotic_count + `" name="antibiotic[` + antibiotic_count + `][sensitivity]" value="Intermediar">
                        <label class="form-check-label sensitivityI" for="sensitivityI` + antibiotic_count + `">I</label>
                    </div>
                    <div class="form-check form-check-inline">
                        <input class="form-check-input" type="radio" id="sensitivityR` + antibiotic_count + `" name="antibiotic[` + antibiotic_count + `][sensitivity]" value="Rezistent">
                        <label class="form-check-label sensitivityR" for="sensitivityR` + antibiotic_count + `">R</label>
                    </div>
                </div>
            </td>
            <td>
                <input type="hidden" name="antibiotic[` + antibiotic_count + `][category_id]" value="` + selectedCategory + `">
                <input type="hidden" name="antibiotic[` + antibiotic_count + `][antibiotic]" value="` + antibiotic.id + `">
                <button type="button" class="btn btn-danger btn-sm delete_row">
                    <i class="fa fa-trash"></i>
                </button>
            </td>
...
//new 3
function add_antibiotic_mc(antibiotics, el, group_microbiology_test_result_id) {
    console.log('group_microbiology_test_result_id:', group_microbiology_test_result_id);
    var antibiotics = JSON.parse(antibiotics);
    ...
    var row = `
        <tr>
            <td>
                <div class="form-group">
                    <select class="form-control antibiotic" name="antibiotic[` + antibiotic_count_mc + `][antibiotic]" required>
                        ` + antibiotics_options + `
                    </select>
                </div>
            </td>
            <td>
                <div class="form-group">
                    <select class="form-control sensitivity" name="antibiotic[` + antibiotic_count_mc + `][sensitivity]" required>
                        <option value="" disabled selected>` + trans("Select Sensitivity 1") + `</option>
                        <option value=".">` + trans(".") + `</option>
                        <option value="Sensitiv">` + trans("Sensitiv") + `</option>
                        <option value="Intermediar">` + trans("Intermediar") + `</option>
            ...
            <td>
                ...
                <input type="" name="antibiotic[` + antibiotic_count_mc + `][group_microbiology_test_result_id]" value="` + group_microbiology_test_result_id + `">
            </td>
        </tr>
    `;
    ...
    // Bind delete_row function to the new delete button
    $(el).closest('table').find('.delete_row').last().click(function () {
        $(this).closest('tr').remove();
    });
}
