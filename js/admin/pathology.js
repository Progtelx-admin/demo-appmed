(function ($) {
  ...
  //visits datatable
  table = $("#pathologys_table").DataTable({
    lengthMenu: [
    ...
    columns: [
      ...
      // {data:"report",sortable:false,orderable:false},
      { data: "reports", sortable: false, searchable: false, orderable: false },
  ...
  $(document).on("click", ".netliva-switch label", function () {
    ...
    $.ajax({
      type: "post",
      url: "/ajax/change_pathology_status/" + id,
  ...
  $("#visit_form").on("submit", function (e) {
    ...
    if (lat == "" || lng == "") {
      e.preventDefault();
      toastr.error(
        trans("Please select home visit location on map"),
        trans("Failed")
      );
    }
  });
