(function($){
    ...
    //active
    $('#healthcertificates').addClass('active');

    //visits datatable
    table=$('#healthcertificates_table').DataTable( {
      ...
      "bSort" : false,
        ...
        "columns": [
            ...
            {data:"visit_type",sortable:false,orderable:false},
            {data:"contract.title",orderable:false,sortable:false},
            {data:"doctor.name",sortable:false,orderable:false},
            {data:"patient.name",sortable:false,orderable:false},
            {data:"patient.phone",sortable:false,orderable:false},
            {data:"patient.national_id",sortable:false,orderable:false},
            {data:"visit_date",sortable:false,orderable:false},
            {data:"protocol_no2",sortable:false,orderable:false},
    ...
    $(document).on('click','.netliva-switch label',function(){
       ...
       $.ajax({
           type:'post',
           url:"/ajax/change_healthcertificate_status/"+id,
   ...
   $('#filter_date').val('');
    ...
    //delete visit
    $(document).on('click','.delete_healthcertificate',function(e){
        e.preventDefault();
        var el=$(this);
        swal({
            title: trans("Are you sure to delete certificate ?"),
            type: "warning",
   ...
   $('#create_patient').on('submit',function(e){
    ...
    //select2 doctors
    $('#select_doctors').select2({
        width:"100%",
        placeholder:trans("Select doctors"),
        ajax: {
           beforeSend:function()
           {
              $('.preloader').show();
    ...
    //select2 antibiotics & therapy
    $('#select_antibiotics').select2({
        width:"100%",
        placeholder:trans("Select antibiotics"),
        ajax: {
           beforeSend:function()
           {
              $('.preloader').show();
           ...
           url: '/ajax/get_antibiotics_select2',
           processResults: function (data) {
                 return {
                       results: $.map(data, function (item) {
                          return {
    ...
    $('#healthcertificate_form').on('submit',function(e){
        var lat=$('#visit_lat').val();
        if(lat==''||lng=='')
        {
            e.preventDefault();
            toastr.error(trans('Please select certificate location on map'),trans('Failed'));
        }
    });
