(function($){
    ...
    table=$('#reports_table').DataTable( {
        ...
        "bSort" : false,
          ...
          "columns": [
            ...
            {data:"model",sortable:false,orderable:false},
            {data:"comment",sortable:false,orderable:false},
    ...
    //active
    $('#instruments').addClass('active');

    //prepare edit user page

    var instrument_branches=$('#instrument_branches').val();

    if(instrument_branches!=null)
    {
        var instrument_branches= JSON.parse(instrument_branches);

        var branches=[];

        instrument_branches.forEach(function(branch){
    ...
    //delete user
    $(document).on('click','.delete_instrument',function(e){
        e.preventDefault();
        var el=$(this);
        swal({
            title: trans("Are you sure to delete instrument ?"),
            type: "warning",
