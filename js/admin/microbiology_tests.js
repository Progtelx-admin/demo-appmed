...
(function($){
    ...
    //active
    $('#microbiology_tests').addClass('active');

    //tests datatable
    table=$('#microbiology_tests_table').DataTable( {
     ...
     <table width="100%">
        ...
        <tbody>
          <tr>
            ...
            <td>
              <button type="button" class="btn btn-danger btn-sm delete_option_mikro">
                <i class="fa fa-trash"></i>
              </button>
            </td>
          </tr>
  ...
  $(document).on("click",".delete_option_mikro",function(){
    ...
    function(){
      ...
      {
        $.ajax({
          url: '/ajax/delete_option_mikro'/+option_id,
          beforeSend:function()
          {
            $('.preloader').show();
            $('.loader').show();
