<nav class="main-header navbar navbar-expand navbar-dark">
  ...
  <ul class="navbar-nav">
    ...
    <li class="nav-item dropdown">
      ...
      <div class="dropdown-menu" aria-labelledby="dropdownMenuButton" style="min-width: 350px">
    ...
    @if(isset($userPos->pointOfSale))
    <li class="nav-item dropdown">
      <form action="{{ route('admin.update_point_of_sale') }}" method="POST">
        @csrf
        <input type="hidden" name="point_of_sale_id" id="selected_point_of_sale_id">
        <button class="btn btn-warning btn-sm dropdown-toggle ml-1" type="button" id="posDropdownButton"
          data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          <i class="fas fa-map-marked-alt"></i> {{ $userPos->pointOfSale->pos_name }} -- {{ $userPos->pointOfSale->cash_in_hand }} EUR
        </button>
        @can('change_pos_nav')
        <div class="dropdown-menu" aria-labelledby="posDropdownButton" style="min-width: 350px">
          @foreach($pointOfSales as $pos)
          <a class="dropdown-item" href="#" onclick="selectPointOfSale({{ $pos->id }})">{{ $pos->pos_name }}</a>
          @endforeach
        </div>
    ...
    @endcan
    <script>
      function selectPointOfSale(pointOfSaleId) {
        document.getElementById('selected_point_of_sale_id').value = pointOfSaleId;
