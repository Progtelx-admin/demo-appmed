...
<div class="row mb-3">
    <div class="col-lg-12">
        ...
        <a @if(isset($previous)) href="{{route('admin.medical_reports.show2',$previous['id'])}}" @endif class="btn btn-info @if(!isset($previous)) disabled @endif">
            <i class="fa fa-backward mr-2"></i> 
            {{__('Previous')}}
        </a>
        <a @if(isset($next)) href="{{route('admin.medical_reports.show2',$next['id'])}}" @endif class="btn btn-success @if(!isset($next)) disabled @endif">
            {{__('Next')}}
...
<form method="POST" action="{{route('admin.medical_reports.pdf2',$group['id'])}}" id="print_form">
    @csrf
    <!-- patient code -->
